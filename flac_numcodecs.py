...
class Flac(Codec):
    ...
    tmpdir : str or Path, optional
        The folder where to save tmp flac files, by default None (default temporaru folder)
    """
    codec_id = "flac"
    max_channels = 2
    supported_dtype_kind = ["i", "u"]

    def __init__(self, compression_level=5, sample_rate=48000, tmpdir=None):
        assert HAVE_PYFLAC, "To use the Flac codec, you need to: >>> pip install pyflac"
        self.tmpdir = tmpdir
    ...
    def _prepare_data(self, buf):
        # checks
        assert buf.dtype.kind in self.supported_dtype_kind, "dtype not supported"
        if buf.ndim == 1:
            data = buf[:, None]
        elif buf.ndim == 2:
